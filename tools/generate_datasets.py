...
#!/usr/bin/env python3
"""
Generate dataset-centric docs from the UKG Pro WFM Data Hub Data Dictionary Excel.

Output:
  docs/datasets/index.md
  docs/datasets/<dataset-slug>.md

This script intentionally ignores:
  - revision history date rows (YYYY-MM-DD)
  - guidance-only header blocks
  - per-column "entity pages" style output
"""
...
def classify_row(row: pd.Series) -> str:
    ...
    # Revision history rows like "2025-08-13"
    if DATE_ROW_RE.match(name):
        return "ignore"

    # Column rows always have a datatype
    if not is_blank(dtype):
        return "column"

    # Dataset header rows: no datatype, but usually have metadata populated
    # (UID and/or Source Pipeline and/or a meaningful description)
    has_dataset_metadata = (not is_blank(uid)) or (not is_blank(pipeline)) or (len(desc) > 0)
    looks_like_dataset_name = (" " not in name)  # vAccrualBalance, vTimecardPunch, m_* etc.
    if has_dataset_metadata and looks_like_dataset_name:
        return "dataset"

    # Group headers: "Detail Dataset", "Accrual Detail Views", etc.
    return "group"
...
def update_group_path(current: Tuple[str, ...], new_group: str) -> Tuple[str, ...]:
    ...
    # Reset top-level when we see something that contains 'Dataset'
    if "dataset" in g.lower():
        return (g,)

    # Otherwise treat as second-level under the last dataset-level group (if present)
    if len(current) >= 1:
        return (current[0], g)

    # If we haven't seen a dataset-level group yet, just use whatever we got
    return (g,)
...
def parse_dictionary(df: pd.DataFrame) -> List[DatasetDoc]:
    ...
    for _, row in df.iterrows():
        ...
        if kind == "column":
            # If columns appear before a dataset header, skip (badly structured section)
            if current_ds is None:
                continue
...
def write_index_page(out_dir: str, datasets: List[DatasetDoc]) -> None:
    ...
    # Group datasets by group_path
    grouped: Dict[Tuple[str, ...], List[DatasetDoc]] = {}
    for ds in datasets:
        grouped.setdefault(ds.group_path, []).append(ds)

    # Sort group keys and datasets
    group_keys = sorted(grouped.keys(), key=lambda k: " → ".join(k).lower())
    for k in group_keys:
        grouped[k] = sorted(grouped[k], key=lambda d: d.name.lower())
...
def main() -> None:
    ...
    # keep only rows where the name column has something
    df = df[df[DICT_COL].notna()].copy()
...
if __name__ == "__main__":
    main()
>>>>>>> df7384c (feat(docs): generate dataset-centric pages from Data Hub dictionary)
